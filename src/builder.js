...
export function mountBuilder(root) {
  ...
  root.innerHTML = `<main class="landing-shell"><header class="landing-header"><p class="brand">qtimer</p><h1>Build a workout.<br />Run it instantly.</h1><p class="landing-lede">Create a simple timer and start moving.</p></header><section class="ai-section"><h2>Use QTimer with AI</h2><p>Paste these instructions into any chat and it can generate a workout with a ready-to-run timer link.</p><button class="copy-instructions-button" type="button">Copy instructions</button><p class="instructions-feedback" aria-live="polite"></p></section><section class="builder" aria-labelledby="builder-title"><label class="field-label" for="workout-title-input">Workout title</label><input id="workout-title-input" class="text-input title-input" maxlength="${WORKOUT_LIMITS.maxTitleLength}" placeholder="Optional title" /><h2 id="builder-title">Build manually</h2><div class="builder-column-labels" aria-hidden="true"><span>Amount</span><span>Unit</span><span>Label</span></div><div class="builder-rows"></div><button class="add-step-button" type="button">+ Add step</button><p class="builder-error" role="alert" hidden></p><button class="primary-button start-workout-button" type="button">Start workout</button><button class="copy-link-button" type="button">Copy shareable workout link</button><p class="copy-feedback" aria-live="polite"></p></section></main>`;
